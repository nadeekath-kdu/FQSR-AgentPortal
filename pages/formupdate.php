...
function handleError($errno, $errstr, $errfile, $errline)
{
    ...
    sendJsonResponse('error', $errorMessage, array(
...
function handleException($e)
{
    ...
    sendJsonResponse('error', 'Server Error: ' . $e->getMessage(), array(
...
try {
    ...
    sendJsonResponse('error', 'Failed to load required files: ' . $e->getMessage());
}
...
try {
    ...
    if ((isset($_POST['inputNic'])) && ($_POST['inputNic'] != NULL) && ($_POST['inputNic'] != "") && ($_POST['inputNic'] != " ")) {
        ...
        if ($applicant_cnt > 0) {
                            ...
                            foreach ($_FILES['documents']['name'] as $key => $filename) {
                                if ($_FILES['documents']['error'][$key] === UPLOAD_ERR_OK) {
                                    ...
                                    if (!in_array($fileType, $allowedTypes)) {
                                        array_push($uploadErrors, "File '$filename' is not an allowed type. Allowed types are PDF, DOCX, JPG, PNG, and GIF.");
                                        continue;
                                    }

                                    // Validate file size
                                    if ($fileSize > $maxSize) {
                                        array_push($uploadErrors, "File '$filename' exceeds the maximum size limit of 5MB.");
                                        continue;
                                    }
            ...
            // Execute the final update statement
            if (!$stmt->execute()) {
                throw new Exception('Failed to update application status: ' . $stmt->error);
            }

            // Prepare success message with document details
        ...
        sendJsonResponse('error', 'Missing or invalid parameters', array('nic' => $enc_nic_no));
    }
} catch (Exception $e) {
    sendJsonResponse('error', 'Application processing error: ' . $e->getMessage());
}
